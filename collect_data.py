...
def fetch_tweets(consumer_key=None, consumer_secret=None, access_key=None, access_secret=None, twitter_handle=None):
    ...
    all_tweets = []
    new_tweets = api.user_timeline(screen_name=twitter_handle, count=300, tweet_mode='extended')
    print("###### Starting to scrape tweets: ######")
    while(len(new_tweets) > 0):
        all_tweets.extend(new_tweets)
        last_id = all_tweets[-1].id - 1
        new_tweets = api.user_timeline(screen_name=twitter_handle, count=300, max_id=last_id, tweet_mode='extended')
...
if __name__ == '__main__':
    ...
    twitter_handle_list = (open('all_handles.txt').read()).split("\n")
    # print(twitter_handle_list)
    for twitter_handle in twitter_handle_list:
        try:
            ...
            print("Something went wrong in getting tweets from " + twitter_handle + " Exception:" + str(e))